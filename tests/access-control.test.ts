...
test(
  ...
  async () => {
    ...
    const [superAdmin, admin, host, contentEditor, finance, support] = await Promise.all([
      ...
      prisma.user.create({
        data: {
          name: "Admin Access Test Content",
          email: `content${emailDomain}`,
          role: "CONTENT_EDITOR",
          passwordHash: "test"
        }
      }),
      prisma.user.create({
        data: {
          name: "Admin Access Test Finance",
      ...
      prisma.user.create({
        data: {
          name: "Admin Access Test Support",
    ...
    const [superAdminToken, adminToken, hostToken, contentToken, financeToken, supportToken] = await Promise.all([
      ...
      createSession(finance.id, `finance-${label}`),
      createSession(support.id, `support-${label}`)
    ]);
