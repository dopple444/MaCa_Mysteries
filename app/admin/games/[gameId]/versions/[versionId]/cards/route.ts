...
import { upsertGameCard } from "../../../../../../lib/admin-rounds";
...
function getCardError(reason: string) {
  switch (reason) {
    case "duplicate-key":
...
export async function POST(
  ...
  if (!(await verifyCsrfToken(formData))) {
    return redirectToGame(request, gameId, "invalid-card");
  }

  const result = await upsertGameCard({
    gameId,
    versionId,
    cardId: getFormValue(formData, "cardId") || undefined,
    roundId: getFormValue(formData, "roundId"),
  ...
  if (!result.ok) {
    return redirectToGame(request, gameId, getCardError(result.reason));
  }

  await logAuditEvent({
    action: result.action === "created" ? "admin.gameCard.created" : "admin.gameCard.updated",
    userId: user.id,
    entityType: "GameCard",
