...
import { queueAccountRecoveryEmailVerification } from "../../../../lib/account-recovery";
...
export async function POST(request: Request, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  const result = await queueAccountRecoveryEmailVerification({ actor, caseId });

  switch (result.status) {
    case "QUEUED":
      return redirectToRecovery(request, { updated: "verification" });
    case "ALREADY_VERIFIED":
      return redirectToRecovery(request, { updated: "already-verified" });
    case "NOT_FOUND":
