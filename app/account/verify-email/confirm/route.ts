...
export async function GET(request: Request) {
  ...
  if (!result.verified) {
    return NextResponse.redirect(createAppUrl("/account/verify-email?error=invalid", request.url), 303);
  }

  const currentUser = await getCurrentUser();
  const destination =
    currentUser?.id === result.userId ? "/account/verify-email?verified=1" : "/login?verified=1";
  return NextResponse.redirect(createAppUrl(destination, request.url), 303);
}
